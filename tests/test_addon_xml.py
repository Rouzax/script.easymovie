...
def test_addon_xml_parses(addon_root):
    ...
    assert addon_root.get('id') == 'script.easymovie'


def test_news_within_schema_length_limit(addon_root):
    """<news> must stay within the schema cap.

    Regression test: v1.3.4 pushed the element to 1666 characters, which
    kodi-addon-checker rejected with "value length cannot be greater than
    1500". Trim the oldest version block when adding a new one.
    """
