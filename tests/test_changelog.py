...
header is user-visible. The sibling addon shipped a release with a
placeholder written over the preceding header, because the release
checklist only verifies the top entry against addon.xml and never looks
further down. These tests check every header, not just the newest one.
"""
import os
import re
